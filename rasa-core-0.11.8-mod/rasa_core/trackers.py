...
class DialogueStateTracker(object):
    ...
    def current_state(self, event_verbosity=EventVerbosity.NONE):
        ...
        # Log the sentiment history 
        sentiments = []
        if evts:
            for e in evts:
            # pp.pprint(e)
                if e['event'] == 'user' and ('tb_polarity' in e['parse_data']):
                    sentiments.append(e['parse_data']['tb_polarity'])
                elif e['event'] == 'user' and not ('tb_polarity' in e['parse_data']):
